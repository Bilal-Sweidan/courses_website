...
export default {
    ...
    methods: {
        getCourseData() {
            axios.get(`http://localhost:8081/student/course/${this.$route.params.courseName}?ID=${this.$route.query.ID}`)
                .then(res => {
        ...
        enroll() {
            axios.post(`http://localhost:8081/student/course/enroll`, { "courseID": this.$route.query.ID })
                .then(res => {
                    if (res.data) {
